%written 1/9/16
function [runtimePGFFA, runtimeFAatOracle, runtimeFAatPoiss, runtimeFAatNegBin, resultDir, n_maxOracle, n_maxPoiss, n_maxNegBin] = icml_runtime_pgffa_vs_trunc(varargin)
    ...
    addParamValue(parser, 'do_plot',       DEFAULT_DO_PLOT)
    ...
    %attempt to open and populate the meta file
	...
	runtimeTot        = 0;
    n_true_record     = zeros(nN, nAlpha, nIter, K);
    y_record          = zeros(nN, nAlpha, nIter, K);
	for iN = 1:nN
		N_hat = N_hatVec(iN);
		...
		for iAlpha = 1:nAlpha
            ...
            fprintf(metaFile, 'N_hat = %d\n', N_hat);
            fprintf(metaFile, 'alpha = %0.2f\n', alpha);
			...
			for iter = 1:nIter
                ...
                %store into the record, keeping the vectors for readability
                n_true_record(iN, iAlpha, iter, :) = n_true;
                y_record(iN, iAlpha, iter, :)      = y;
                ...
                %record this experiment in the meta file
                fprintf(metaFile, 'Iteration %d of %d:\n', iter, nIter);
                fprintf(metaFile, 'n = [%d', n_true(1));
                    for iDisp = 2:K; fprintf(metaFile, ', %d', n_true(iDisp)); end
                    fprintf(metaFile, ']\n');
                fprintf(metaFile, 'y = [%d', y(1));
                    for iDisp = 2:K; fprintf(metaFile, ', %d', y(iDisp)); end
                    fprintf(metaFile, ']\n');
                fprintf(metaFile, 'll_pgffa = %0.2f\n', llPGFFA);
                fprintf(metaFile, 'll_fa = %0.2f\n', llFA);
                fprintf(metaFile, 'time_pgffa = %0.2f\n', runtimePGFFA(iN, iAlpha, iter));
                fprintf(metaFile, 'time_fa_oracle = %0.2f\n', runtimeFAatOracle(iN, iAlpha, iter));
                fprintf(metaFile, 'time_fa_poiss = %0.2f\n', runtimeFAatPoiss(iN, iAlpha, iter));
                fprintf(metaFile, 'time_fa_negbin = %0.2f\n', runtimeFAatNegBin(iN, iAlpha, iter));
                fprintf(metaFile, 'converged = %d\n', converged);
                fprintf(metaFile, '\n');
			end
    ...
    %%write the sampled data (n and y) to file
    data_file = fullfile(resultDir, 'data.mat');
    %rename data for readability/compatability in file
    n      = n_true_record;
    y      = y_record;
    rho    = alphaVec;
    N_hat  = N_hatVec;
    gamma  = unscaledGamma;
    lambda = N_hatVec' * unscaledGamma;
%     delta  = delta;
    
    save(data_file, 'n', 'y', 'rho', 'N_hat', 'gamma', 'lambda', 'delta');
end